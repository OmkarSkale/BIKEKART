...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM parts_cat";
...
if(isset($_POST["parts"])){
	...
	$product_query = "SELECT * FROM parts LIMIT $start,$limit";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['part_id'];
			$pro_cat   = $row['part_cat'];
			$pro_brand = $row['part_brand'];
			$pro_title = $row['part_title'];
			$pro_price = $row['part_price'];
			$pro_image = $row['part_img'];
			$pro_link  = $row['part_desc'];
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-body '>
									...
									<img src='product_images/$pro_image' style='width:auto ; height: auto; max-width:310px; max-height: 170px;'/>
...
if(isset($_POST["get_seleted_Category"]) || isset($_POST["selectBrand"]) || isset($_POST["search"])){
	if(isset($_POST["get_seleted_Category"])){
		$id = $_POST["cat_id"];
		$sql = "SELECT * FROM parts WHERE part_cat = '$id'";
	}else if(isset($_POST["selectBrand"])){
		$id = $_POST["brand_id"];
		$sql = "SELECT * FROM parts WHERE part_brand = '$id'";
	}else {
		$keyword = $_POST["keyword"];
		$sql = "SELECT * FROM parts WHERE part_keywords LIKE '%$keyword%'";
	}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								...
								<div class='panel-body'>
									<img src='product_images/$pro_image' style='width:310px; height:170px;'/>
								</div>
								<div class='panel-heading'>Rs.$pro_price.00
									<button pid='$pro_id' style='float:right;' id='product' class='btn btn-danger btn-xs'>AddToCart</button>
								</div>
	...
	if (isset($_POST["getCartItem"])) {
		...
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				$product_title = $row["part_title"];
				$product_price = $row["part_price"];
				$product_image = $row["part_img"];
				$cart_item_id = $row["id"];
				$qty = $row["qty"];
				echo '
					<div class="row">
